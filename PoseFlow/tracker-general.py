...
Project: AlphaPose
File Created: Thursday, 18st Dec 2018 14:55:41 pm
...
def display_pose(imgdir, visdir, tracked, cmap):
    for imgname in tqdm(tracked.keys()):
            ...
            if pose.shape[0] == 16:
                ...
                for idx in range(len(pairs)):
                    plt.plot(np.clip(pose[pairs[idx],0],0,width),np.clip(pose[pairs[idx],1],0,height), 'r-',
                            color=cmap(tracked_id), linewidth=50*np.mean(pose[pairs[idx],2]),  alpha=np.mean(pose[pairs[idx],2]))
                ...
                for idx in range(len(pairs)):
                    plt.plot(np.clip(pose[pairs[idx],0],0,width),np.clip(pose[pairs[idx],1],0,height),'r-',
                            color=cmap(tracked_id), linewidth=5*np.mean(pose[pairs[idx],2]), alpha=0.12*np.mean(pose[pairs[idx],2]))
        plt.axis('off')
        ax = plt.gca()
        ax.set_xlim([0,width])
        ...
        if not os.path.exists(visdir): 
            os.mkdir(visdir)
        fig.savefig(os.path.join(visdir,imgname),pad_inches = 0.0, bbox_inches=extent, dpi=13)
        plt.close()
