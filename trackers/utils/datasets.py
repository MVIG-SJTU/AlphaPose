...
class JointDataset(LoadImagesAndLabels):  # for training
    def __init__(self, root, paths, img_size=(1088,608), augment=False, transforms=None,train=True):
        ...
        for ds, path in paths.items():
            ...
            '''
            self.label_files[ds] = [x.replace('images', 'labels_with_ids_train').replace('.png', '.txt').replace('.jpg', '.txt')
                                for x in self.img_files[ds]]
            '''
            self.label_files[ds] = []
            mode = 'train' if self.train else 'sub_val'
            for x in self.img_files[ds]:
                prev = x[:31]
                rear = x[32:]
                rear = rear.replace('/',chr(ord('/')+65248)).replace('jpg','txt')
                prev = prev.replace('images','labels_with_ids_{}'.format(mode))
                self.label_files[ds].append(os.path.join(prev,rear))
