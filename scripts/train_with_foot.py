...
def validate(m, opt, heatmap_to_coord, batch_size=20):
    ...
    res = evaluate_mAP(os.path.join(opt.work_dir, 'test_kpt.json'), ann_type='keypoints', ann_file='person_keypoints_val2017_foot.json')
    return res['AP']
...
def validate_gt(m, opt, cfg, heatmap_to_coord, batch_size=20):
    ...
    with open(os.path.join(opt.work_dir, 'test_gt_kpt.json'), 'w') as fid:
        json.dump(kpt_json, fid)
    res = evaluate_mAP(os.path.join(opt.work_dir, 'test_gt_kpt.json'), ann_type='keypoints', ann_file = 'person_keypoints_val2017_foot.json')
    return res['AP']
