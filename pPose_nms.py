...
def write_json_tmp(all_results, outputpath, for_eval=False):
    '''
    all_result: result dict of predictions
    outputpath: output directory
    '''
    ...
    if form == 'cmu': # the form of CMU-Pose/OpenPose
        with open(os.path.join(outputpath,'alphapose-results-tmp.json'), 'w') as json_file:
            json_file.write(json.dumps(json_results_cmu))
            if not os.path.exists(os.path.join(outputpath,'sep-json-tmp')):
                os.mkdir(os.path.join(outputpath,'sep-json-tmp'))
            for name in json_results_cmu.keys():
                with open(os.path.join(outputpath,'sep-json-tmp',name.split('.')[0]+'.json'),'w') as json_file:
                    json_file.write(json.dumps(json_results_cmu[name]))
    else:
        with open(os.path.join(outputpath,'alphapose-results-tmp.json'), 'w') as json_file:
            json_file.write(json.dumps(json_results))