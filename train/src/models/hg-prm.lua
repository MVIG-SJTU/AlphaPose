...
local function hourglass(n, f, nModules, inp, inputResH, inputResW, type, B, C)
    ...
    for i = 1,nModules do up1 = ResidualUp(f,f,1,type,false,inputResH,inputResW,B,C)(up1) end

    -- Lower branch
    local low1 = nnlib.SpatialMaxPooling(2,2,2,2)(inp)
    for i = 1,nModules do low1 = ResidualDown(f,f,1,type,false, inputResH/2, inputResW/2,B,C)(low1) end
    local low2

    if n > 1 then low2 = hourglass(n-1,f,nModules,low1,inputResH/2,inputResW/2,type,B,C)
    else
        low2 = low1
        for i = 1,nModules do low2 = ResidualDown(f,f,1,type,false,inputResH/2,inputResW/2,B,C)(low2) end
    end

    local low3 = low2
    for i = 1,nModules do low3 = ResidualDown(f,f,1,type,true,inputResH/2,inputResW/2,B,C)(low3) end
...
function createModel()
    ...
    local inputResH = opt.inputResH/4
    local inputResW = opt.inputResW/4
    ...
    local r1 = ResidualPyramid(64,128,1,'no_preact',false,opt.inputResH/2,opt.inputResW/2, B,C)(cnv1)
    local pool = nnlib.SpatialMaxPooling(2,2,2,2)(r1)                       -- 64
    --local r4 = ResidualPyramid(128,128,1,'preact',false,inputRes,B,C)(pool)
    local r4 = ResidualPyramid(128,128,1,'preact',false,inputResH,inputResW,B,C)(pool)
    --local r5 = ResidualPyramid(128,opt.nFeats,1,'preact',false,inputRes,B,C)(r4)
    local r5 = ResidualPyramid(128,opt.nFeats,1,'preact',false,inputResH,inputResW,B,C)(r4)
    ...
    for i = 1,opt.nStack do
        ...
        local hg = hourglass(4, opt.nFeats, opt.nResidual, inter, inputResH, inputResW,'preact',B,C)
