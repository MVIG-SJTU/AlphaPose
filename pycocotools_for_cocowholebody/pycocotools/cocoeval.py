        ...
        for gt in gts:
            ...
            if p.iouType == 'keypoints':
                if len(gt['keypoints']) % 133 == 0 or len(gt['keypoints']) % 136 == 0:
                    _ignore = gt['ignore']
                    gt['ignore'] = dict()
                    body_kp = np.array(gt['keypoints'][0:17*3])
                    foot_kp = np.array(gt['keypoints'][17*3:(17+6)*3])
                    face_kp = np.array(gt['keypoints'][(17+6)*3:(17+6+68)*3])
                    hand_kp = np.array(gt['keypoints'][(17+6+68)*3:399])
        ...
        for part in parts:
            ...
            for j, gt in enumerate(gts):
                ...
                if part == 'body':
                    ...
                    g = np.array(gt['keypoints'][(body_kpnum + foot_kpnum + face_kpnum)*3:399],dtype=np.float32)
                    #assert g.shape[0] == 42*3, g.shape
                else:
                    g = np.array(gt['keypoints'][0:399],dtype=np.float32)
                ...
                for i, dt in enumerate(dts):
                    if part == 'body':
                        ...
                        d = np.array(dt['keypoints'][(body_kpnum + foot_kpnum + face_kpnum)*3:399],dtype=np.float32)
                        #assert d.shape[0] == 42*3, d.shape
                    else:
                        d = np.array(dt['keypoints'][0:399], dtype=np.float32)
